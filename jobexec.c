...
char **getShellParams(char **params)
{
    ...
    assertnlog2(cmd != NULL, "Failed to malloc for shell execution of",
		params[1]);
...
void createOutputDir(int argc, char *argv[])
{
    ...
    for (l = 1; l < argc; l++) {
	i = 0;
	while (argv[l][i] != '\0' && j < (MAXCMDHASHLEN - 1)) {
	    c = argv[l][i];
	    if (c >= 'A' && c <= 'Z'
		|| c >= 'a' && c <= 'z' || c >= '0' && c <= '9') {
		cmdline[j] = c;
	...
	assertnlog2(!chmod(dir, S_IRWXU | S_IRWXG | S_IRWXO | S_ISVTX),
		    "chmod BASEDIR", dir);
    }
...
#ifdef SYSLOG_ON
	    syslog(LOG_ERR, "%s failed to execute. Output: %s", argv[1],
		   dir);
#endif
